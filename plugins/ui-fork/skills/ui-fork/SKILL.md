...
Default deliverable package for `guide` and `system` modes contains two separate artifacts:

- `design-guide.md`: the human-readable design system guide using `templates/output_template.md`.
- `design-tokens.yaml`: the implementation-oriented token draft using `templates/tokens_template.yaml`.

In chat, output these as two clearly separated fenced blocks or sections. Do not merge the YAML token artifact into the Markdown as prose. Only create actual files when the user explicitly asks to save, generate, create, update, or persist them.
...
Use `templates/output_template.md` for the Markdown guide and `templates/tokens_template.yaml` for the separate YAML token artifact. Output the full design specification:

- Design Brief
- Design Guide
- Prompt Contract
- Confidence Split
- Separate `design-tokens.yaml` artifact
...
Use the same two-artifact package as `guide`, plus stronger implementation detail:

- Full standalone `design-tokens.yaml` using `templates/tokens_template.yaml`.
...
Use `templates/tokens_template.yaml` for the separate YAML artifact shape. Tokens are draft implementation hints, not final truth.
...
- Keep token output machine-readable YAML; put explanation in `design-guide.md`, not as loose prose inside `design-tokens.yaml`.
...
Only write files when the user asks to save, generate, create, update, or persist the design guide or tokens.

When writing files:

1. Default filenames are `DESIGN.md` and `design-tokens.yaml`.
2. Check whether either file already exists.
3. If a target file does not exist, create it directly.
4. If a target file exists, do not overwrite silently. Ask whether to overwrite, append, or merge.
5. Use the same fixed Markdown structure as `templates/output_template.md` for `DESIGN.md`.
6. Use `templates/tokens_template.yaml` for `design-tokens.yaml`.
7. Include YAML front matter in `DESIGN.md` only if the user asks for machine-readable metadata or if the existing `DESIGN.md` already uses front matter.
